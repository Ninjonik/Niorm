...
def dict_to_values_string(data):
    keys = ", ".join(data.keys())
    values = ", ".join(str(val) if type(val) == str else
                       str(int(val)) if type(val) == int else
                       str(float(val)) if type(val) == float else
                       "true" if val == True else "false" if val == False else
...
def dict_to_sql_where(d):
    def format_value(v):
        if isinstance(v, bool):
            ...
            return f"'{v}'"
    return " AND ".join("{}={}".format(k, format_value(v)) for k, v in d.items())
